...
ggplot(oat_score_df, aes(x = `Constr. comp. 1`, y = `Constr. comp. 2`,
  ...
  labs(
    x = glue::glue("Constrained comp. 1 ({MVA.synt(oat_rda)[[2]]$tab[1,2] %>% round(2)}%)"),
    y = glue::glue("Constrained comp. 2 ({MVA.synt(oat_rda)[[2]]$tab[2,2] %>% round(2)}%)")
  ) +
  # Theme
  theme_bw() +
...
ggplot(oat_load_df) +
  ...
  geom_label_repel(
    ...
    segment.alpha = 0.6,
    segment.size = 0.3,
    nudge_x = oat_load_df$xplus * .01,
    nudge_y = oat_load_df$yplus * .01,
    seed = 678
  ) +
  labs(
    x = glue::glue("Constrained comp. 1 ({MVA.synt(oat_rda)[[2]]$tab[1,2] %>% round(2)}%)"),
