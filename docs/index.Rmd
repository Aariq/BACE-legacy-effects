...
library(lmtest)
library(targets)
set.seed(2785) #for consistency with permutation tests
```
...
When the RDA was significant, we followed up with univariate ANOVAs for log-transformed concentrations of Fe, Zn, N, **Ca, Mg, and K. Only factors found significant in the RDA were included in the post-hoc ANOVA models and only statistically significant ANOVA models (false discovery rate adjusted Wald test p-values < 0.05) were analyzed.**
...
-   **14.99%** of variation in nutrients explained by "house"
-   **29.38%** of variation in nutrients explained by historical \* current + jul_herbivory + late_herbivory
-   Of the "constrained" variance, **57.29%** is explained with the fist two axes.

```{r}
anova(kale_rda)
```

An omnibus permutation test shows that the overall RDA is significant (**$F = 2.11$**, $p < 0.001$).
...
The first constrained component (the one that explains most of the variation due to predictors) explains about **57%** of the variation explained by the predictors (less if you'd rather report the total variation in nutrients explained) (Figure \@ref(fig:rda)a,b).
