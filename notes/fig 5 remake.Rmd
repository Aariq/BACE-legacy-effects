...
all_nutr <- 
  ...
  map(
    ~ .x %>%
        select(current, historical, plot, plot_id, pot_number, pot_id, N = n, Fe = fe, Zn = zn, Ca = ca, K = k, Mg = mg)
    ) %>%
  bind_rows(.id = "species") %>% 
  #convert ppm to percent (Fe and Zn are measured in ppm, the others in %)
