...
all_nutr <- 
  ...
  map(
    ~ .x %>%
        select(current, historical, plot, plot_id, pot_number, pot_id, N = n, Fe = fe, Zn = zn, Ca = ca, K = k)
    ) %>%
...
p_kale <-
  ...
  theme_bw() +
  labs(x = "Historical", y = "Concentration (units??)")
p_bean <-
...
Would be nice to put legend in empty space.  Snagged this from a [stackoverflow post](https://stackoverflow.com/questions/54438495/shift-legend-into-empty-facets-of-a-faceted-plot-in-ggplot2)
...
```{r}
p_kale <- shift_legend(p_kale)
p_bean <- shift_legend(p_bean)
p_oat  <- shift_legend(p_oat)
p_kale
p_bean
p_oat
```

I like this.  Tidy up colors and factor names and such
